...
/**
 * High-performance lock-free buffer using LMAX Disruptor.
 *
...
public class DisruptorBuffer<T> extends Blocks.Source<T> implements ProcessorBlock<T, T> {
    ...
    private final EventTranslatorOneArg<MutableMessage<T>, T> translator;

    private volatile Disruptor<MutableMessage<T>> disruptor;
    private BufferBatch<T> batch;
    ...
    public DisruptorBuffer(Config config) {
        ...
        Disruptor<MutableMessage<T>> disruptor = new Disruptor<>(
            MutableMessage::new,
            ringBufferSize,
        ...
        disruptor.setDefaultExceptionHandler(config.getExceptionHandler());
        disruptor.handleEventsWith(new RingBufferEventHandler());

        this.disruptor = disruptor;

        // Adapter for Disruptor's EventTranslator interface
        this.translator = (MutableMessage<T> message, long seq, T event) -> message.set(event, null);
    }
    ...
    public void postBatch(@Nonnull Collection<? extends T> elements) {
        ...
        disruptor.publishEvents(translator, (T[]) arr.toArray());
    }
     ...
     */
    private class RingBufferEventHandler implements SequenceReportingEventHandler<MutableMessage<T>>, BatchStartAware, LifecycleAware {
         ...
         */
        @Override
        public void onEvent(MutableMessage<T> holder, long sequence, boolean endOfBatch) {
            final T event = holder.getValue();
