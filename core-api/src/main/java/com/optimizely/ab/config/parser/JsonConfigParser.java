...
final public class JsonConfigParser implements ConfigParser {
    ...
    public ProjectConfig parseProjectConfig(@Nonnull String json) throws ConfigParseException {
        try {
            ...
            if (rootObject.has("localHoldouts")) {
                localHoldouts = parseHoldouts(rootObject.getJSONArray("localHoldouts"));
            } else {
    ...
    private List<Holdout> parseHoldouts(JSONArray holdoutJson) {
        ...
        for (int i = 0; i < holdoutJson.length(); i++) {
            ...
            // Parse optional includedRules field: null = global holdout, array = local holdout
